extension PredicateNet {
    ...
    public func markingGraph(from marking: MarkingType) -> PredicateMarkingNode<T>? {
        // on definit l'ensemble des transitions, l'ensemble des marquages a renvoyer et une liste des transitions bindé
          let T = self.transitions
          let Pm = PredicateMarkingNode<T>(marking: marking, successors: [:])
          var reste = [Pm]
          var m_ok = [Pm]
          var B = [PredicateTransition<T>.Binding]()
          //on repete l'operation jusqu'a ce qu'il n'y aie plus de marquage a traiter
          while (reste.last) != nil{
            let M = reste.popLast()
                m_ok.append(M!)
                let m_courent = M!.marking
            for i in T{                       // on tire les transitions bindés
              B = i.fireableBingings(from: m_courent)
                var new_successor : PredicateBindingMap<T> = [:]
                //on itere sur tous les binding, on tire une transition, dans le premier if, on borne le marquage et
                //dans le deuxieme, on verifie qu'il n'y ait pas deux meme marquages
                for bind in B{
                  if let tirage = i.fire(from: m_courent, with: bind) {
                    let new_M = PredicateMarkingNode<T>(marking: tirage,successors: [:])
                    if (m_ok.contains(where:{PredicateNet.greater(new_M.marking,$0.marking)}) == true){
                      return nil
                    }
                    if (m_ok.contains(where:{PredicateNet.equals($0.marking, new_M.marking)}) == false){
                      reste.append(new_M)
                      m_ok.append(new_M)

                      new_successor[bind] = new_M
                      M!.successors.updateValue(new_successor, forKey: i)
                    }
                  }
                }
              }
